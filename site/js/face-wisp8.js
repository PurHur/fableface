// face-wisp8.js — WISP VIII: ANIMA. The fully configurable flagship.
// Everything PORTRAIT (VII) does, taken further on all three axes at once —
// cinematic, effects-dense, and lifelike — with EVERY knob exposed:
//   CONFIG       ~30 named parameters (this.cfg, schema in Wisp8Face.PARAMS)
//                promoted from hardcoded constants to live uniforms — driven
//                from the UI config panel + FableFace.setParam()/preset() API.
//   ANAMORPHIC   a horizontal lens streak on the brightest highlights — the
//                single most "cinema" cue a hologram can carry.
//   SOFT BLOOM   two-iteration wide bloom for a filmic, non-blobby glow.
//   THEMEABLE    the base hologram colour is a uniform — cyan by default, but
//                amber / viridian / magenta presets are one call away.
//   KEY + RIM    the portrait key, warm counter-rim and fresnel are all dialed.
...
// horizontal anamorphic streak: a wide normalized 1D blur of the bright-pass.
const STREAK_FS = `#version 300 es
precision highp float;
in vec2 vUv;
uniform sampler2D uTex;
uniform vec2 uStep; // (spread/w, 0)
out vec4 outColor;
void main(){
  vec3 c = texture(uTex, vUv).rgb*0.20;
  float wsum = 0.20;
  for (int i = 1; i <= 12; i++){
    float w = exp(-float(i)*0.26);
    c += texture(uTex, vUv + uStep*float(i)).rgb*w;
    c += texture(uTex, vUv - uStep*float(i)).rgb*w;
    wsum += 2.0*w;
  }
...
uniform float uBloomK, uAnamK, uChroma, uGrain, uVig, uWarmth;
uniform vec3 uLift;
out vec4 outColor;
vec3 aces(vec3 x){
...
void main(){
  // configurable radial chromatic aberration (0.0021 @ chroma 0.45 ≈ VII)
  vec2 off = (vUv - 0.5)*uChroma*0.0047;
  vec3 col;
  col.r = texture(uScene, vUv + off).r;
  col.g = texture(uScene, vUv).g;
  col.b = texture(uScene, vUv - off).b;
  col += texture(uBloom, vUv).rgb*uBloomK;                        // soft filmic bloom
  col += texture(uStreak, vUv).rgb*uAnamK*vec3(0.55, 0.80, 1.25); // anamorphic lens streak
  col = aces(col*uExposure);
  col += uLift*0.9*(1.0 - col);                                   // shadow lift (grade)
  float l = dot(col, vec3(0.299, 0.587, 0.114));
  col = mix(col, col*vec3(0.93, 1.0, 1.06), (1.0 - l)*0.5*uWarmth);  // teal shadows
  col += vec3(0.05, 0.025, 0.0)*smoothstep(0.6, 1.0, l)*uWarmth;     // warm highlights
  col = mix(vec3(l), col, uSat);                       // saturation
  col = (col - 0.5)*uCon + 0.5;                        // contrast
  col += (grain(vUv*vec2(1920.0, 1080.0)) - 0.5)*0.032*uGrain*(1.0 - l); // grain, shadow-weighted
  float vig = 1.0 - uVig*0.63*pow(length((vUv - 0.5)*vec2(1.5, 1.4)), 2.4);
...
// ---- WISP VIII live config ----
uniform vec3 uBaseCol;
uniform float uKey, uRimK, uWarmRimK, uCatch, uIrisK;
uniform float uMotionK, uBreathK, uStormK, uChladniK, uShimmerK, uSparkK;
uniform float uScanK, uSepK, uSkinA, uFogK, uDofK, uRingsOn;
...
void main(){
  ...
  float motion = (1.0 - 0.92*uFreeze)*uMotionK;
  ...
  vec3 base = mix(uBaseCol, uEmoCol, clamp(uGlow*1.7 + uEmoPulse*0.4, 0.0, 0.92));
  vec3 holo = base;
  ...
  if (isIris) {
    ...
    base += vec3(1.0, 1.0, 1.05)*catchL*uCatch;
    ...
    if (fx == 2) { // RAIN CITY: falling rain streaks around the head
      ...
      vec3 ov = vec3(cos(oang)*orad, sin(oang)*orad, 0.0);
      float oinc = 0.35 + plane*0.42;                          // inclination per plane
      float oci = cos(oinc), osi = sin(oinc);
      ov = vec3(ov.x, ov.y*oci - ov.z*osi, ov.y*osi + ov.z*oci); // tilt about X
      float opre = uTime*0.06*motion + plane*1.9;              // slow precession about Y
      float opc = cos(opre), ops = sin(opre);
      ov = vec3(ov.x*opc + ov.z*ops, ov.y, -ov.x*ops + ov.z*opc);
      p = ov + vec3(0.0, 0.02, 0.0);
      float olead = step(0.88, fract(aSeed*61.0));             // brighter lead motes
    ...
    if (uLevel > 0.012 && !inner) {
      ...
      p += aNrm*F*sin(uTime*14.0 + F*2.0)*0.0035*lv*motion*uChladniK;
      chlG = (exp(-22.0*F*F) - 0.32)*lv*uChladniK; // nodal contrast: lines glow, antinodes dim (net luminance ~conserved)
    }
    ...
    p += aNrm*shim*0.0007*(0.35 + 0.65*uCohR)*motion*uShimmerK*(isContour ? 0.3 : 1.0);
  ...
  if (!isIris && !inner) {
    ...
    slice = mix(1.0, 0.72 + 0.36*pow(0.5 + 0.5*sin(slPhase), 1.6), uScanK);
  ...
  float depthF = mix(1.0, 0.50 + 0.50*smoothstep(-0.085, 0.045, p.z), uFogK);
  vec3 world = isRing ? (p + uHeadPos*0.4 + vec3(0.0, -0.012, 0.0)) : (uHeadRot*p + uHeadPos);

  float chan = floor(fract(aSeed*91.7)*3.0);
  float sep = (0.0028*uLevel + 0.004*uReactW + 0.002*uDoubt + 0.0018*uGestureW + (fx == 2 ? 0.0024 : 0.0))*((isIris || inner) ? 0.0 : 1.0)*uSepK;
  ...
  if (!isIris) {
    ...
    base *= mix(vec3(1.0), mask, inner ? 0.25 : (isRing ? 0.15 : 0.60)); // orbital motes stay coherent
  }
  ...
  float rim = isCore ? 1.0 : (0.34 + 1.55*fres*(0.55 + 0.50*keyFace*uKey)*uRimK + 0.18*keyFace*uKey);
  rim *= 1.0 + shim*(0.05 + 0.09*uCohR)*uShimmerK + 0.04*uPinkG; // collective shimmer + 1/f life
  ...
  if (!isIris && !isRing) {
    ...
    float stormAmp = (isCore ? 1.4 : isCircuit ? 0.6 : 0.18)*(fx == 4 ? 1.6 : 1.0)*uStormK;
  ...
  float tw = 0.75 + (0.30 + 0.45*clamp(uEnergy, 0.3, 1.7))*uSparkK*hash11(aSeed*997.0 + floor(uTime*(5.0 + 3.0*uEnergy) + aSeed*20.0));
  if (fx == 1) tw *= 0.8 + 0.6*hash11(aSeed*77.0 + floor(uTime*11.0)); // stardust scintillation
  float breathAmp = 0.92 + 0.08*uBreath*uBreathK;
  ...
  vFade = vFade0*lifeA*ta*(spark > 0.5 ? 0.45 : 1.0)*(feed > 0.5 ? 0.6 : 1.0)*backFade*mirrorFade*lidar*(isRing ? uRingsOn : 1.0);

  // DEPTH OF FIELD: focus plane at the eyes; blur = bigger + dimmer
  float blur = clamp(abs(-viewPos.z - uFocusD)*9.0, 0.0, 1.6)*uDofK;
  vFade /= (1.0 + blur*0.9);
...
export class Wisp8Face {
  static title = 'WISP VIII';
  static tech = 'ANIMA · CONFIGURABLE';
  static CAM = { dist: 0.92, targetY: -0.012, fov: 22 }; // 85mm-portrait compression
  static blurb =
    'The configurable flagship. Everything the PORTRAIT face does, pushed further on ' +
    'all three axes — a wider filmic bloom with an anamorphic lens streak, a themeable ' +
    'hologram colour, a stronger portrait key with brighter catchlights. And every knob ' +
    'is live: open TUNE (top-right) or call FableFace.setParam() / preset().'

  // ---- CONFIG: the live look. Defaults are the WISP VIII "much better" look;
  //      resetConfig() returns here. Every value flows to a shader uniform. ----
  static CFG = {
    // cinema
    exposure: 1.06, bloom: 0.62, bloomThresh: 0.72, anamorphic: 0.55, chroma: 0.45,
    grain: 0.5, vignette: 0.6, saturation: 1.08, contrast: 1.07, warmth: 0.6,
    // hologram
    baseCol: [0.20, 0.80, 1.00], scanlines: 0.8, trails: 0.80, separation: 1.0,
    density: 1.0, skinOpacity: 0.62,
    // light
    key: 1.15, rim: 1.05, warmRim: 0.6, catchlight: 3.0, iris: 1.15,
    // life
    motion: 1.0, breath: 1.0, storms: 1.0, chladni: 1.0, shimmer: 1.0, sparkle: 1.0,
    // depth
    dof: 1.0, fog: 1.0,
    // toggles
    shell: true, rings: true, reflection: true,
  };

  // UI metadata: grouped controls (main.js builds the TUNE panel + API from this).
  static PARAMS = [
    { group: 'Cinema', items: [
      { key: 'exposure', label: 'Exposure', min: 0.4, max: 1.8, step: 0.02 },
      { key: 'bloom', label: 'Bloom', min: 0, max: 1.3, step: 0.02 },
      { key: 'bloomThresh', label: 'Bloom threshold', min: 0.3, max: 1.2, step: 0.02 },
      { key: 'anamorphic', label: 'Anamorphic streak', min: 0, max: 1.5, step: 0.02 },
      { key: 'chroma', label: 'Chromatic aberration', min: 0, max: 1.5, step: 0.02 },
      { key: 'grain', label: 'Film grain', min: 0, max: 1.5, step: 0.02 },
      { key: 'vignette', label: 'Vignette', min: 0, max: 1.5, step: 0.02 },
      { key: 'saturation', label: 'Saturation', min: 0, max: 1.7, step: 0.02 },
      { key: 'contrast', label: 'Contrast', min: 0.6, max: 1.5, step: 0.02 },
      { key: 'warmth', label: 'Teal / orange', min: 0, max: 1.5, step: 0.02 },
    ] },
    { group: 'Hologram', items: [
      { key: 'baseCol', label: 'Base colour', type: 'color' },
      { key: 'scanlines', label: 'Scanlines', min: 0, max: 1.5, step: 0.02 },
      { key: 'trails', label: 'Trail persistence', min: 0, max: 0.93, step: 0.01 },
      { key: 'separation', label: 'RGB separation', min: 0, max: 2.2, step: 0.05 },
      { key: 'density', label: 'Particle size', min: 0.5, max: 1.7, step: 0.02 },
      { key: 'skinOpacity', label: 'Skin opacity', min: 0.2, max: 1.3, step: 0.02 },
    ] },
    { group: 'Light', items: [
      { key: 'key', label: 'Key light', min: 0, max: 2, step: 0.02 },
      { key: 'rim', label: 'Rim / fresnel', min: 0.2, max: 2.2, step: 0.02 },
      { key: 'warmRim', label: 'Warm counter-rim', min: 0, max: 1.5, step: 0.02 },
      { key: 'catchlight', label: 'Catchlights', min: 0, max: 5, step: 0.1 },
      { key: 'iris', label: 'Iris glow', min: 0, max: 2.5, step: 0.05 },
    ] },
    { group: 'Life', items: [
      { key: 'motion', label: 'Motion', min: 0, max: 1.6, step: 0.02 },
      { key: 'breath', label: 'Breathing', min: 0, max: 2, step: 0.05 },
      { key: 'storms', label: 'Cortical storms', min: 0, max: 2, step: 0.05 },
      { key: 'chladni', label: 'Voice resonance', min: 0, max: 2, step: 0.05 },
      { key: 'shimmer', label: 'Coherence shimmer', min: 0, max: 2, step: 0.05 },
      { key: 'sparkle', label: 'Sparkle / dust', min: 0, max: 2, step: 0.05 },
    ] },
    { group: 'Depth', items: [
      { key: 'dof', label: 'Depth of field', min: 0, max: 2, step: 0.05 },
      { key: 'fog', label: 'Depth fog', min: 0, max: 1.5, step: 0.05 },
    ] },
    { group: 'Toggles', items: [
      { key: 'shell', label: 'Glass shell', type: 'toggle' },
      { key: 'rings', label: 'Scene rings', type: 'toggle' },
      { key: 'reflection', label: 'Floor reflection', type: 'toggle' },
    ] },
  ];

  // named looks (FableFace.preset(name)); each is a partial cfg overlay.
  static PRESETS = {
    anima:     {}, // the default (reset)
    cinematic: { bloom: 0.5, anamorphic: 0.85, chroma: 0.6, grain: 0.75, vignette: 0.9, warmth: 1.0, contrast: 1.12, scanlines: 0.4, skinOpacity: 0.72, catchlight: 3.4 },
    hologram:  { bloom: 0.8, anamorphic: 0.3, chroma: 0.25, grain: 0.25, scanlines: 1.25, separation: 1.6, trails: 0.86, baseCol: [0.20, 0.82, 1.0], warmth: 0.25, fog: 1.2 },
    neon:      { baseCol: [0.85, 0.25, 1.0], bloom: 0.95, anamorphic: 0.7, saturation: 1.4, scanlines: 1.1, separation: 1.9, warmth: 0.2, catchlight: 3.6 },
    vivid:     { baseCol: [0.15, 1.0, 0.75], bloom: 0.85, saturation: 1.5, contrast: 1.15, anamorphic: 0.6, sparkle: 1.6, storms: 1.4 },
    minimal:   { bloom: 0.3, anamorphic: 0.15, chroma: 0.12, grain: 0.2, scanlines: 0.25, separation: 0.4, vignette: 0.4, skinOpacity: 0.85, rings: false, storms: 0.5 },
    amber:     { baseCol: [1.0, 0.62, 0.22], bloom: 0.6, warmth: 1.1, anamorphic: 0.6, scanlines: 0.7, saturation: 1.15 },
  };

  static _clamp(cfg) {
    const flat = Wisp8Face.PARAMS.flatMap(g => g.items);
    for (const it of flat) {
      if (it.type === 'color' || it.type === 'toggle') continue;
      if (typeof cfg[it.key] === 'number') cfg[it.key] = Math.min(it.max, Math.max(it.min, cfg[it.key]));
    }
    return cfg;
  }

  setCfg(partial) {
    if (!partial || typeof partial !== 'object') return this.cfg;
    for (const k in partial) {
      if (!(k in this.cfg)) continue;
      if (k === 'baseCol' && Array.isArray(partial[k])) this.cfg[k] = partial[k].slice(0, 3).map(Number);
      else if (typeof this.cfg[k] === 'boolean') this.cfg[k] = !!partial[k];
      else if (typeof partial[k] === 'number' && isFinite(partial[k])) this.cfg[k] = partial[k];
    }
    Wisp8Face._clamp(this.cfg);
    return this.cfg;
  }

  constructor(gl, assets) {
    this.gl = gl;
    this.cfg = JSON.parse(JSON.stringify(Wisp8Face.CFG)); // live, mutable copy
    ...
    const bg = program(gl, HOLO_BG_VS, HOLO_BG_FS, 'wisp8.bg');
    this.bgProg = bg.prog; this.bgU = bg.u;
    this.quadVao = fullscreenVAO(gl);
    const pt = program(gl, PT_VS, PT_FS, 'wisp8.pt');
    this.ptProg = pt.prog; this.ptU = pt.u;
    const decay = program(gl, QUAD_VS, DECAY_FS, 'wisp8.decay');
    this.decayProg = decay.prog; this.decayU = decay.u;
    const comp = program(gl, QUAD_VS, COMP_FS, 'wisp8.comp');
    this.compProg = comp.prog; this.compU = comp.u;
    const shell = program(gl, QUAD_VS, SHELL_FS, 'wisp8.shell');
    this.shellProg = shell.prog; this.shellU = shell.u;
    const bright = program(gl, QUAD_VS, BRIGHT_FS, 'wisp8.bright');
    this.brightProg = bright.prog; this.brightU = bright.u;
    const blur = program(gl, QUAD_VS, BLUR_FS, 'wisp8.blur');
    this.blurProg = blur.prog; this.blurU = blur.u;
    const fin = program(gl, QUAD_VS, FINAL_FS, 'wisp8.final');
    this.finProg = fin.prog; this.finU = fin.u;
    const streak = program(gl, QUAD_VS, STREAK_FS, 'wisp8.streak');
    this.streakProg = streak.prog; this.streakU = streak.u;
    ...
    this._streakT = null; this._streakF = null; // anamorphic streak (shares bloom res)
  ...
  _ensureTrails(w, h) {
    ...
    if (bw !== this._bw || bh !== this._bh) { // bloom ping-pong + streak (quarter res)
      ...
      if (this._streakT) { gl.deleteTexture(this._streakT); gl.deleteFramebuffer(this._streakF); }
      const st = this._mkBuf(bw, bh, this._hdr);
      this._streakT = st.tex; this._streakF = st.fbo;
    }
  }
  ...
  draw(cm) {
    ...
    const shellOn = this._dtAvg < 0.07 && cfg.shell; // adaptive: skip raymarch below ~14fps (or toggled off)
    ...
    gl.uniform1f(this.decayU.uDecay, cfg.trails);
    ...
    gl.uniform1f(u.uPixelScale, this._fh / (2 * cm.tanHalf) * cfg.density);
    ...
    // ---- WISP VIII live config → uniforms ----
    gl.uniform3f(u.uBaseCol, cfg.baseCol[0], cfg.baseCol[1], cfg.baseCol[2]);
    gl.uniform1f(u.uKey, cfg.key);
    gl.uniform1f(u.uRimK, cfg.rim);
    gl.uniform1f(u.uWarmRimK, cfg.warmRim);
    gl.uniform1f(u.uCatch, cfg.catchlight);
    gl.uniform1f(u.uIrisK, cfg.iris);
    gl.uniform1f(u.uMotionK, cfg.motion);
    gl.uniform1f(u.uBreathK, cfg.breath);
    gl.uniform1f(u.uStormK, cfg.storms);
    gl.uniform1f(u.uChladniK, cfg.chladni);
    gl.uniform1f(u.uShimmerK, cfg.shimmer);
    gl.uniform1f(u.uSparkK, cfg.sparkle);
    gl.uniform1f(u.uScanK, cfg.scanlines);
    gl.uniform1f(u.uSepK, cfg.separation);
    gl.uniform1f(u.uSkinA, cfg.skinOpacity);
    gl.uniform1f(u.uFogK, cfg.fog);
    gl.uniform1f(u.uDofK, cfg.dof);
    gl.uniform1f(u.uRingsOn, cfg.rings ? 1 : 0);
    gl.bindVertexArray(this.vao);
    ...
    // ---- bloom chain: bright-pass → anamorphic streak → 2× gaussian H/V ----
    ...
    gl.uniform1f(this.brightU.uThresh, cfg.bloomThresh);
    gl.drawArrays(gl.TRIANGLES, 0, 3);

    // anamorphic streak: wide horizontal smear of the bright-pass (bloomT[0])
    gl.bindFramebuffer(gl.FRAMEBUFFER, this._streakF);
    gl.useProgram(this.streakProg);
    gl.bindTexture(gl.TEXTURE_2D, this._bloomT[0]);
    gl.uniform1i(this.streakU.uTex, 0);
    gl.uniform2f(this.streakU.uStep, 2.6 / this._bw, 0);
    gl.drawArrays(gl.TRIANGLES, 0, 3);

    // two blur octaves for a soft, non-blobby bloom
    gl.useProgram(this.blurProg);
    gl.uniform1i(this.blurU.uTex, 0);
    for (let oct = 0; oct < 2; oct++) {
      const spread = oct === 0 ? 1 : 2;
      gl.bindFramebuffer(gl.FRAMEBUFFER, this._bloomF[1]);
      gl.bindTexture(gl.TEXTURE_2D, this._bloomT[0]);
      gl.uniform2f(this.blurU.uDir, spread / this._bw, 0);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      gl.bindFramebuffer(gl.FRAMEBUFFER, this._bloomF[0]);
      gl.bindTexture(gl.TEXTURE_2D, this._bloomT[1]);
      gl.uniform2f(this.blurU.uDir, 0, spread / this._bh);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
    }

    // ---- final: tonemap + config grade + streak + grain + vignette to screen ----
    const lift = (cm.scene && cm.scene.grade && cm.scene.grade.lift) || [0, 0.01, 0.02]; // scene mood tint
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.viewport(0, 0, cm.width, cm.height);
    ...
    gl.bindTexture(gl.TEXTURE_2D, this._streakT);
    gl.uniform1i(this.finU.uStreak, 2);
    gl.uniform1f(this.finU.uExposure, cfg.exposure);
    gl.uniform1f(this.finU.uSat, cfg.saturation);
    gl.uniform1f(this.finU.uCon, cfg.contrast);
    gl.uniform1f(this.finU.uTime, cm.t);
    gl.uniform1f(this.finU.uBloomK, cfg.bloom);
    gl.uniform1f(this.finU.uAnamK, cfg.anamorphic);
    gl.uniform1f(this.finU.uChroma, cfg.chroma);
    gl.uniform1f(this.finU.uGrain, cfg.grain);
    gl.uniform1f(this.finU.uVig, cfg.vignette);
    gl.uniform1f(this.finU.uWarmth, cfg.warmth);
    gl.uniform3f(this.finU.uLift, lift[0], lift[1], lift[2]);
    gl.drawArrays(gl.TRIANGLES, 0, 3);
    gl.bindVertexArray(null);
    gl.activeTexture(gl.TEXTURE0);
  }
