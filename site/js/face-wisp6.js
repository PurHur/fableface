// face-wisp6.js — WISP VI: CINEMA ENGINE.
// Everything GLASS MIND is, finished like a film. The generational leap is
// CRAFT, not another gimmick:
//   HDR PIPELINE  particles accumulate in float buffers — no more clamp-to-
//                 white; a real bright-pass gaussian BLOOM; ACES filmic
//                 tonemap; per-scene COLOR GRADE (lift/sat/contrast/exposure).
//   LIVING LINES  contour particles carry a flow parameter — brightness
//                 pulses race along the actual feature edges of the face.
//   DEEP IRIS     two parallax depth layers, counter-rotating segmented arcs.
//   + the driver got VISEMES (AH/OO/EE/closure mouth shapes), Duchenne smile
//   eye-narrowing, and gaze micro-tremor — all faces benefit, VI shows it off.
// Inherits everything from V: anatomy layers, brain-core storms, conduits,
// glass shell, DOF, scenes/adornments, emotion shockwaves, intensity coupling.
...
void main(){
  ...
  col += (grain(vUv*vec2(1920.0, 1080.0)) - 0.5)*0.014; // film grain
...
void main(){
  ...
  if (isContour) amp2 = tw*moire*(1.5 + flowPulse*2.2 + sweep*0.8 + ripple*2.2 + storm*1.4 + pulse*1.8 + emoBand*2.2)*(0.9 + uLevel*0.4 + uGlow*0.28 + uGestureW*0.25)*depthF;
...
export class Wisp6Face {
  static title = 'WISP VI';
  static tech = 'CINEMA ENGINE · HDR';
  static CAM = { dist: 0.62, targetY: -0.012 };
  static blurb =
    'The glass mind, finished like a film. Particles accumulate in high dynamic range — ' +
    'no more white clipping — through a true bloom chain, an ACES filmic tonemap and a ' +
    'per-scene color grade. Feature lines carry flowing light. The iris gained parallax ' +
    'depth and segmented arcs. And the driver now articulates visemes: watch the mouth ' +
    'round, widen and close as it speaks. This is the face the companion deserves.';
  ...
  constructor(gl, assets) {
    ...
    const bg = program(gl, HOLO_BG_VS, HOLO_BG_FS, 'wisp6.bg');
    this.bgProg = bg.prog; this.bgU = bg.u;
    this.quadVao = fullscreenVAO(gl);
    const pt = program(gl, PT_VS, PT_FS, 'wisp6.pt');
    this.ptProg = pt.prog; this.ptU = pt.u;
    const decay = program(gl, QUAD_VS, DECAY_FS, 'wisp6.decay');
    this.decayProg = decay.prog; this.decayU = decay.u;
    const comp = program(gl, QUAD_VS, COMP_FS, 'wisp6.comp');
    this.compProg = comp.prog; this.compU = comp.u;
    const shell = program(gl, QUAD_VS, SHELL_FS, 'wisp6.shell');
    this.shellProg = shell.prog; this.shellU = shell.u;
    const bright = program(gl, QUAD_VS, BRIGHT_FS, 'wisp6.bright');
    this.brightProg = bright.prog; this.brightU = bright.u;
    const blur = program(gl, QUAD_VS, BLUR_FS, 'wisp6.blur');
    this.blurProg = blur.prog; this.blurU = blur.u;
    const fin = program(gl, QUAD_VS, FINAL_FS, 'wisp6.final');
